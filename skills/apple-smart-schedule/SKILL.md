...
- **行程内部的细节也合并进总览**：讲座/会议的精确时段、地点、会议室号、线上会议号、议程要点——都进总览事件的**备注**或**地点**字段，**不新建讲座/会议子事件**，也不单独为它建提醒。判准：凡是「出差总览事件已经开始/结束的当天或中间一天发生的细节」，都按合并处理；只有真正独立、不依附任何出行总览的日程，才新建事件。已写入 DEC-006。
