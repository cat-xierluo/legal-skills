...
- [ ] 评估"开 worktree 前 3 查"是否可脚本化（`pre-worktree-check.sh`），目前文档级已覆盖主流程
- [ ] 评估"PR 创建后 mergeable 检查"是否可前置到"开 worktree 前"，避免基于过期 main 开 worktree
...
## v1.5.0 增量（2026-07-11）

实战来源：vision-extract 模型池项目（PR #45）合并。

- [x] §2 Worktree 加"开 worktree 前必做 3 查"块（防 base 过期导致 PR 报 not mergeable + DECISIONS 编号撞车）
- [x] §2 加"本地独有 commit 未 push 的处理"（push / merge origin/main / reset 3 选 + 禁止擅自 reset）
- [x] §4 PR 工作流加"自 PR 自 review 限制"子节（GitHub 不允许自 approve；自 PR 用 `gh pr merge --squash --delete-branch` 绕过；cleanup warning 说明）
- [x] 新增 §10 "多 worktree 并行与 main worktree 占用"（3 方案：方案 A 推荐主仓库不 attach main / 方案 B main 单独 worktree / 方案 C 临时释放）
- [x] DEC-015 实战教训记录
