/**
 * html2pptx - Convert HTML slides to pptxgenjs presentation with positioned elements
 *
 * USAGE (single page):
 ...
 *
 * USAGE (multi-page from single HTML):
 *   const pptx = new pptxgen();
 *   pptx.layout = 'LAYOUT_16x9';
 *   const results = await html2pptxMulti('slides.html', pptx);
 *   // results = [{ slide, placeholders }, ...]
 *   await pptx.writeFile('output.pptx');
 *
 * FEATURES:
 ...
 *   - Auto-detects multi-page HTML (sections with .slide class or <section> tags)
 *   - Forces viewport to match PPTX layout for viewport-relative HTML (100vh, clamp, vw)
 *
 * OPTIONS:
 *   forceViewport: true   - Force viewport to match PPTX layout dimensions
 *   slideSelector: '.slide' - CSS selector for slide containers (multi-page mode)
 *   lenient: false         - Skip strict validation errors (warn instead of throw)
 *
 * RETURNS (single page):
 *   { slide, placeholders, warnings }
 *
 * RETURNS (multi-page):
 *   [{ slide, placeholders, warnings }, ...]
 */
...
async function html2pptx(htmlFile, pres, options = {}) {
  ...
  try {
    ...
    // Determine target viewport from PPTX layout
    let targetWidth, targetHeight;
    if (viewportWidth && viewportHeight) {
      targetWidth = viewportWidth;
      targetHeight = viewportHeight;
    } else if (pres.presLayout) {
      targetWidth = (pres.presLayout.width / EMU_PER_IN) * PX_PER_IN;
      targetHeight = (pres.presLayout.height / EMU_PER_IN) * PX_PER_IN;
    } else {
      // Default 16:9
      targetWidth = 960;
      targetHeight = 540;
    ...
    try {
      ...
      if (forceViewport) {
        // Force viewport to match PPTX layout dimensions
        await page.setViewportSize({
          width: Math.round(targetWidth),
          height: Math.round(targetHeight)
        });

        // Inject CSS to force body dimensions (handles viewport-relative HTML)
        await page.evaluate((w, h) => {
          const style = document.createElement('style');
          style.textContent = `
            html { height: auto !important; overflow: hidden !important; scroll-snap-type: none !important; }
            body { width: ${w}px !important; height: ${h}px !important; overflow: hidden !important; margin: 0 !important; padding: 0 !important; }
          `;
          document.head.appendChild(style);
        }, targetWidth, targetHeight);

        // Wait for layout to settle after forced dimensions
    ...
    if (bodyDimensions.errors && bodyDimensions.errors.length > 0) {
      if (lenient) {
        warnings.push(...bodyDimensions.errors);
      } else {
    ...
    if (textBoxPositionErrors.length > 0) {
      if (lenient) {
        warnings.push(...textBoxPositionErrors);
      } else {
    ...
    if (slideData.errors && slideData.errors.length > 0) {
      if (lenient) {
        warnings.push(...slideData.errors);
      } else {
        validationErrors.push(...slideData.errors);
      }
    }

    // Only throw on validation errors in strict mode
    if (!lenient && validationErrors.length > 0) {
    ...
    return { slide: targetSlide, placeholders: slideData.placeholders, warnings };
  } catch (error) {
...
/**
 * html2pptxMulti - Convert multi-page HTML to PPTX (one slide per section)
 *
 * Detects slide containers (default: `.slide` or `section` elements) in a single
 * HTML file and converts each to a separate PPTX slide.
 *
 * @param {string} htmlFile - Path to HTML file
 * @param {object} pres - PptxGenJS instance
 * @param {object} options - Same as html2pptx, plus:
 *   slideSelector: CSS selector for slide containers (default: auto-detect)
 * @returns {Array<{slide, placeholders, warnings}>}
 */
async function html2pptxMulti(htmlFile, pres, options = {}) {
  const {
    slideSelector = null,
    ...restOptions
  } = options;

  const tmpDir = restOptions.tmpDir || process.env.TMPDIR || '/tmp';
  ...
  try {
    ...
    // Detect slide containers and their dimensions
    const slideInfo = await page.evaluate(({ sel }) => {
      // Auto-detect slide containers
      let containers;
      if (sel) {
        containers = document.querySelectorAll(sel);
      } else {
        // Try common selectors in order of priority
        containers = document.querySelectorAll('.slide, section[class*="slide"]');
        if (containers.length === 0) {
          containers = document.querySelectorAll('section');
        }
        if (containers.length === 0) {
          // Single page - entire body
          return { multiPage: false, count: 1, slideWidth: 0, slideHeight: 0 };
        }
      }

      // Detect actual slide dimensions from the first container
      const firstSlide = containers[0];
      const computed = window.getComputedStyle(firstSlide);
      const slideWidth = parseFloat(computed.width) || firstSlide.offsetWidth;
      const slideHeight = parseFloat(computed.height) || firstSlide.offsetHeight;

      return { multiPage: true, count: containers.length, slideWidth, slideHeight };
    }, { sel: slideSelector });

    if (!slideInfo.multiPage) {
      // Single page HTML, use normal conversion
      await browser.close();
      const result = await html2pptx(htmlFile, pres, { ...restOptions, forceViewport: restOptions.forceViewport !== false });
      return [result];
    }

    // Determine target viewport from detected slide dimensions
    let targetWidth = slideInfo.slideWidth;
    let targetHeight = slideInfo.slideHeight;

    // Fallback if detection failed
    if (!targetWidth || !targetHeight) {
      if (restOptions.viewportWidth && restOptions.viewportHeight) {
        targetWidth = restOptions.viewportWidth;
        targetHeight = restOptions.viewportHeight;
      } else if (pres.presLayout) {
        targetWidth = (pres.presLayout.width / EMU_PER_IN) * PX_PER_IN;
        targetHeight = (pres.presLayout.height / EMU_PER_IN) * PX_PER_IN;
      } else {
    ...
    console.log(`检测到 ${slideInfo.count} 个幻灯片，尺寸: ${targetWidth}×${targetHeight}px`);

    // Multi-page: isolate each slide container and convert individually
    const results = [];
    for (let i = 0; i < slideInfo.count; i++) {
      // Re-open page for each slide to get clean state
      const slidePage = await browser.newPage();
      slidePage.on('console', (msg) => {
        console.log(`Browser console: ${msg.text()}`);
      });

      try {
        await slidePage.goto(`file://${filePath}`);
        await slidePage.setViewportSize({ width: Math.round(targetWidth), height: Math.round(targetHeight) });

        // Isolate the target slide: hide all others, show only this one
        const selector = slideSelector || '.slide, section[class*="slide"], section';
        await slidePage.evaluate(({ sel, idx, w, h }) => {
          // Inject override styles using detected dimensions
          const style = document.createElement('style');
          style.textContent = `
            html { height: auto !important; overflow: hidden !important; scroll-snap-type: none !important; }
            body { width: ${w}px !important; height: ${h}px !important; overflow: hidden !important; margin: 0 !important; padding: 0 !important; }
          ...
          containers.forEach((c, ci) => {
            if (ci === idx) {
              // Copy slide's own background to body so extractSlideData picks it up
              const slideBg = window.getComputedStyle(c).backgroundColor;
              const slideBgImage = window.getComputedStyle(c).backgroundImage;
              if (slideBg && slideBg !== 'rgba(0, 0, 0, 0)') {
                document.body.style.backgroundColor = slideBg;
              }
              if (slideBgImage && slideBgImage !== 'none') {
                document.body.style.backgroundImage = slideBgImage;
              }

              // Make this slide fill the viewport using detected dimensions
              c.style.cssText = `
        ...
        }, { sel: selector, idx: i, w: targetWidth, h: targetHeight });

        await slidePage.waitForTimeout(100);

        // Extract slide data from the isolated page
        const slideData = await extractSlideData(slidePage);
        const targetSlide = pres.addSlide();

        const warnings = [...(slideData.errors || [])];
        if (warnings.length > 0 && !restOptions.lenient) {
          console.warn(`Slide ${i + 1} warnings:`, warnings);
        }

        await addBackground(slideData, targetSlide, tmpDir);
        addElements(slideData, targetSlide, pres);

        results.push({ slide: targetSlide, placeholders: slideData.placeholders, warnings });
      } finally {
...
module.exports.html2pptxMulti = html2pptxMulti;