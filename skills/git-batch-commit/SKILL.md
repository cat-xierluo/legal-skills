...
version: "1.2.5"
license: MIT
description: "智能 Git 批量提交工具，将混合文件修改按类型（docs、feat、fix、chore、config 等）自动分类并创建多个聚焦提交，使用 Conventional Commits 格式（英文前缀加中文内容）。本技能应在用户说 git 提交、批量提交、拆分提交、整理提交时使用，或当暂存的文件涉及多个类别需要分开提交时使用。"
---
...
6. **Subtree 推送检查** - 检查本次提交是否涉及已注册的 subtree 子项目（详见下方「提交后检查：Subtree 推送」章节）
   - **前置条件**：仅在项目目录下存在 `skills/subtree-publish/config/subtree-skills.json` 时才执行此步骤
   - 如果配置文件不存在，**直接跳到第7步，不提示、不输出任何信息**
   - 存在时：检查本次提交的文件是否命中清单中的子目录，如有则提示用户是否推送
7. **完成** - 获得清晰历史的聚焦提交
...
## 提交后检查：ClawHub 同步（第5步）

> **前置条件**：仅当 `skills/clawhub-sync/` 存在时执行，否则静默跳过。

提交完成后，检查是否有 skill 版本升级、新增 MIT 技能、或白名单中尚未同步的 skill。命中任一条件时提示用户是否同步。详细流程参见 [clawhub-sync SKILL.md](../clawhub-sync/SKILL.md)。

## 提交后检查：Subtree 推送（第6步）

> **前置条件**：仅当 `skills/subtree-publish/config/subtree-skills.json` 存在时执行，否则静默跳过。

检查本次提交是否涉及已注册的 subtree 子项目，命中时提示用户是否推送到独立仓库。详细流程参见 [subtree-publish SKILL.md](../subtree-publish/SKILL.md)。
