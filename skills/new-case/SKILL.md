...
description: 将案件/咨询/商标/专利材料整理成标准化目录结构。支持诉讼案件（12目录）、潜在项目/咨询（2目录）、商标案件（7个平铺分类）、专利案件（按业务子类型选 3 个模板：申请 8 目录/复审无效 9 目录/检索其他 5 目录，目录顺序按项目进展）四种预设。本技能应在用户需要创建新案件、初始化案件目录结构、整理咨询材料、接收商标/专利新业务、或通过参数和自然语言指定案件编号、委托人、案件类型等信息快速创建案件时使用。不要用于：单独生成法律文书、进行法律研究、证据分析等非案件初始化任务。
...
- **商标案件 (trademark)**：知产业务，覆盖商标注册 / 异议 / 驳回复审 / 无效宣告 / 变更转让；7 个平铺分类目录（委托材料/商标图样/申请书/官方文书/证据材料/律师工作文件/发票财务），多个申请号的同类文件归位到一起，生成项目信息卡和续展提醒待办
- **专利案件 (patent)**：知产业务，覆盖发明 / 实用新型 / 外观设计 / 无效 / 复审 / 检索 / PCT / 变更 / 年费 10 种子业务；**单一预设下挂 3 个业务子模板**：(1) 申请模板（8 目录，发明/实用新型/外观/PCT）、(2) 复审/无效模板（9 目录，复审/无效，含证据+对方提交）、(3) 检索/其他模板（5 目录，检索/布局/变更/年费）。**目录顺序按项目进展**：委托 → 工作记录 → 清单/客户提供 → 客户提供/律师工作 → 律师工作/申请书 → 国知局文件 → 发票/财务。**律师工作与文件进程合并**（都是律师产出）。每个项目按业务子类型选模板，**不创建空目录**。生成项目信息卡和年费提醒待办。
...
**商标案件**：创建 7 个平铺分类目录，多个申请号的同类文件归位到一起：
```
├── [项目编号] 项目信息.md
├── [项目编号] 待办事项.md
...
商标预设的 workflow 特殊点：执行前必须先询问用户提供**主申请号**和**关联申请号**列表（逗号分隔），用于填入项目信息卡的"关联申请号"表和识别每份文书归属。
...
1. **识别需要补日期的文件**：扫描目标目录的 PDF 文件
   - 跳过已带 `YYMMDD ` 前缀的文件（如 `260424 商标注册申请不予受理通知书_90752080.pdf`）
   - 跳过非 PDF（图片/Word）
   - 跳过明显是律师起草而非官方文书的文件
...
5. **重命名**：
   ...
   - 格式：`YYMMDD 原文件名.pdf`（如 `250710 84549280_NOVA ARTS_驳回通知书.pdf`）
...
**实战案例**（伊甸文化项目 2026-06-09）：
- `84549280_NOVA ARTS_驳回通知书.pdf` → 提取到 `2025年07月10日` → 改名 `250710 84549280_NOVA ARTS_驳回通知书.pdf`
- `商标驳回复审决定书_84549280_第28类NOVAARTS_176717.pdf` → 提取到文末 `2025年12月29日` → 改名 `251229 商标驳回复审决定书_84549280_第28类NOVAARTS_176717.pdf`
...
- `assets/trademark.yaml` — 商标案件（7 个平铺分类目录）
