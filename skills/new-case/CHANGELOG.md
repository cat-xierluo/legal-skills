...
- 增加个人目录规范学习流程：可从用户直接描述或多个已有案件目录的名称、层级和文件位置归纳个人习惯。
- 新增 `references/local-conventions.md`，规定只读盘点、明示规则/稳定规律/候选规则分层、变更草案、用户确认、配置验证和失败回退。
- 新增 `config/local-conventions.example.yaml`，定义个人覆盖配置的通用格式；真实值写入被仓库 `.gitignore` 覆盖的 `config/local-conventions.local.yaml`。

### 改进

- 明确配置优先级为“用户本次明确指令 > 个人覆盖配置 > 内置预设”。
- 个人学习仅在用户明确要求记住或更新规范时持久化；普通建档不暗中学习，不改写公开 `assets/*.yaml`。

### 技术优化

- 覆盖合并采用确定性规则：映射递归合并、标量替换、列表整段替换，避免目录列表的猜测式合并。

## [1.4.0] - 2026-08-14

### 变更

- **诉讼档案模板 v3.0 → v4.0（Task-016，对齐 case-progress 数据契约）**：
  - 输出改为固定文件名 `00 - 📅 日程管理/case.yaml`（无编号前缀，DEC-011；唯一状态真值，建档后修改须经 case_store CLI）
  - 结构对齐 schema v4.0：meta（标识四分：案件短码/律所案号/法院案号/目录标识）、状态二分（生命周期状态 / 程序阶段＋锁定）、法定期限列表化（补起算日/法律依据/抵消标记）、**任务并入 yaml 唯一真源**、行级 `source: user|ai`、新增 上下文/同步 节
  - 删除派生性章节：工作记录摘要/进展摘要/风险预警/案件文档状态/数据分析（看板实时派生不落库）；项目管理.里程碑 → 任务＋时间线；预算跟踪 → 费用信息；工时分类统计 → 工时记录.md
  - 占位规范：`null` 替代 `[待确认]`/空串；枚举去 emoji；日期强制 YYYY-MM-DD
- `templates/task-list.md` 降级为便签视图（任务真源在 case.yaml 的 任务 节）
- `assets/litigation.yaml`：core_files 与 management_files 输出改为固定名（案件信息.md 在根；工时记录.md、case.yaml、待办事项.md 在 00）
- **同日增补（8 案件材料盘点 + CaseBoard 对照驱动，schema 未发布不另立版）**：meta 增 `业务领域`（诉讼|商标|专利|咨询|其他非诉，类型档案路由键）；新增 关联案件[] / 开庭与听证[] / 审级记录[] / 其他诉讼参与人[] / 扩展信息{} 五节；程序阶段加 侦查|审查起诉；律师费加 计费方式；证据索引加 取证方式。非诉扩展机制见 schema §7（profile 惰性立项）
- 契约来源：case-progress skill `references/schema.md` v4.0（消费项目 SuitAgent 方案 v1.6，决策 #045/#046）

### 设计演进

- 历次模板漂移（v2.1.0 英文 → v3.0 中文 → 手工自创）造成消费项目 5 套案件数据格式并存；v4.0 为 canonical 定稿，存量案件由消费项目 M4 迁移（.legacy 归档），本 skill 不回溯
...
- 实际验证：某商标异议项目（异议模板 8 目录）、某商标注册项目（注册模板 7 目录）干跑+应用（客户名与案号略）
...
- **vs 商标 7 目录的关键差异**：
  ...
  - 11 发票/财务（含年费追踪）
- **统一编号**：区别于某历史项目的 1/2/3/4/5/9/10/11 跳跃编号，**新项目统一用 1-11 连续编号**。详见 DEC-006。
...
- **2026-06-08 晚**（基于某商标项目（客户名略）干跑验证）：早期设计曾尝试"项目共享 + 申请号子目录"两层结构（每个申请号一个子目录，目录内 7 分类），实测对 2-3 个申请号的常见场景属过度设计。改为 **7 个平铺分类目录**，多申请号同类文件归位到一起，申请号信息以"关联申请号"表形式记录在项目信息卡中。详见 DEC-003。
- **2026-06-09**（基于该项目 v2 验证）：发现两类实现 bug：(1) `cp` 未带 `-p` 导致文件 mtime 全部丢失；(2) 把用户自建的 `DEMO MARK/` 子目录内容拉平到 `02 商标图样/` 根目录，破坏用户原有组织。已写为 SKILL.md 第三步"操作铁律"和 classification-guide.md 分类原则 4、5。
- **2026-06-09**（基于某商标项目 04 官方文书 PDF 优化）：发现 2 个官方文书 PDF（驳回通知书、驳回复审决定书）文件名无发文日期，导致按文件名排序时乱序。**新增"第三步半：官方文书 PDF 自动加发文日期前缀"**：扫描 `04/03/05` 中无 `YYMMDD` 前缀的 PDF，用 `pdftotext` 或 OCR 提取发文日期，匹配"发文日期/决定日期"等关键字附近的日期，`mv` 改名（保留 mtime）。示例：`12345678_DEMO MARK_驳回通知书.pdf` → `250710 12345678_DEMO MARK_驳回通知书.pdf`；`商标驳回复审决定书_12345678_第9类DEMARK_000001.pdf` → `251229 商标驳回复审决定书_12345678_第9类DEMARK_000001.pdf`。详见 DEC-005。
