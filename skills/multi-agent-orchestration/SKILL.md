...
2. **目标目录不是 git 仓**：`spawn-worker.sh` 检测到 `--project` 不在 git work tree 内，自动切轻量并打印 `SPAWN_WORKER_LIGHTWEIGHT_AUTO` 通知（仍可被 `--branch` 缺省等线索识别）。
...
- `METADATA.json` 记 `isolation_mode: "lightweight"`，`branch / base_ref / base_sha` 留空。
