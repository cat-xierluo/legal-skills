...
|---------|--------------------------|----------|------|
| Claude Code | `glm-5.2`（常规/高端）· `MiniMax-M3`（简单+多模态/图片） | 默认主力 host；**按任务路由（非轮换）**，同 provider ≤3 并发，溢出到 qoder/codebuddy | 详细 provider 映射见 `config/claude-provider-registry.example.json` |
| Codex | （见 §2.4 codex_policy） | 用户明确要求时 | 智能高额度贵，默认不主动派 |
| OpenCode | `opencode:<provider>/<model>` | OpenCode 已有额度 | 按 OpenCode profile |
| `qoderclicn`（QoderWork CN） | `qoder-3.7MAX`、`qoder-3.7PLUS` | 用户主动要求 / 主力并发打满溢出（与 codebuddy 是两个不同平台/额度，并行避并发） | SDK 变量需清理 |
| `codebuddy`（WorkBuddy/CodeBuddy） | `deepseek-v4-pro`、`deepseek-v4-flash` | 用户主动要求 / 主力并发打满溢出（与 qoderclicn 是两个不同平台/额度，并行避并发） | 默认带 `-y` |
...
- **配置路径**：`config/orchestration-personal.json`（**随 skill 走，在仓库内**——当前用户的实际路由偏好；不放在 `~/.claude/`，避免配置与 skill 割裂）。
...
| `main_force.task_routing.high_end` | 常规 / 高端任务的 model | `glm-5.2` |
| `main_force.task_routing.simple_multimodal` | 简单任务（尤其多模态 / 图片解读）的 model | `MiniMax-M3` |
| `main_force.task_routing.default` | 任务类型不明时回落 | `glm-5.2` |
| `concurrency.max_per_provider` | 同一 provider 并发上限（尽量更低，避免限流连累其他任务） | `3` |
| `concurrency.overflow_strategy` | 超上限时溢出到哪些 backend（不同平台 / 额度，互不抢并发） | `codebuddy / qoderclicn` |
