...
- 既有 Worktree 预门禁（Task-116，Badminton Lab 实测事故②）：Orca 模式下 spawn 在任何 provider lease / Orca worktree create / Session Context / terminal / dispatch 副作用之前，先机械判定 exact branch 是否已被同 repo worktree 占用或本地分支已存在（Orca create 会自动改用 `-2` 后缀新建）、exact `--worktree` 路径是否已存在（同仓 worktree/错仓/脏工作区/非 git 目录等未知状态一律视为不可新建）。命中即稳定输出 `EXISTING_WORKTREE_REQUIRES_RECOVERY` 并 exit 3，零副作用零残留；已有 Worktree 的恢复必须走 reauthorize/quota-park/明确恢复入口，spawn 不做自动复用，路径不存在的新建流程不变。回归：`scripts/test-spawn-worker-orca.sh` 的 Task-116 预门禁矩阵。
- worktree 落盘后、任何 terminal/worker-start/任务注入之前，`spawn-worker.sh` 先跑 isolation pre-gate：非 lightweight 且非 dry-run 时机械判定 worktree 目录存在、`branch --show-current == 预期分支` 且 HEAD 可解析。该门禁是既有 Worktree 预门禁之后的兜底层：仅当 runtime 漂移仍造成 mismatch（如同名分支只存在于 origin 时 Orca 漂移生成 `-2` 后缀分支）才触发，打印 `SPAWN_WORKER_ISOLATION_PREGATE_FAILED` 并 exit 2——此时调用日志里没有任何 terminal create/run-create/task-create/worker-start，Session Context 与 authority receipt 尚未落盘，worktree 保留供 PM 精确清理；不存在"worker 已带任务活跑、spawn 却报失败"的 partial dispatch。final `SPAWN_WORKER_GATE` 只保留 launch 后才能观察的 pane cwd 校验。回归：`scripts/test-spawn-worker-orca.sh` 末尾的 Orca `-2` 后缀 E2E。
...
liveness 预门禁（Task-116，Badminton Lab 实测事故①）：任何 mutation（含 run-use 回绑与 METADATA 改写）之前，命令先读权威 Dispatch 状态（`worker-show`）。只有未 released/acked/settled 的 live 目标可继续；已进入结算链（worker_done → release → ack → settled）或状态不可证明（调用失败、Dispatch 记录缺失）的目标稳定输出 `REAUTHORIZE_NOT_LIVE` 并零副作用拒绝——授权文件、launch.sh、终端、METADATA 全部不变，不再"先建替换终端才发现死目标再回滚"。已结算目标的恢复不经由 reauthorize：先按正常收口 release/ack，再走 quota-park 或明确恢复入口；原 Task-113 的注册期 TASK_REUSED 复位链降级为预门禁与注册之间状态漂移的 fail-closed 兜底（漂移/unknown 仍不复位）。
