...
if grep -Fq 'ctx-auto' "$T77_SENDS" && grep -Fq -- '--type worker_done' "$T77_SENDS" \
  ...
  && grep -Fq -- '--dispatch-capability' "$T77_SENDS" \
  && grep -Fq -- 'orchestration ask' "$T77_SENDS" && grep -Fq -- 'ask --resume' "$T77_SENDS" \
  && grep -Fq -- 'orchestration check --terminal' "$T77_SENDS"; then
  ok "Task-077 injected line carries native capability worker_done plus ask/resume/check forms"
else
  bad "Task-077 injected line carries native capability worker_done plus ask/resume/check forms"
fi
