...
' 2>/dev/null) || completion_command=""
      ...
      printf -v inject_text \
        '[dispatch-bind 自动补绑] task_id=%s dispatch_id=%s。完成时原样执行且只执行一次以下命令（首次权限拒绝后立即停止，不得改写或包装）: %s。阻塞时: orca orchestration ask --from %s --question "阻塞问题" --timeout-ms 600000；超时只用回执 message ID 执行 ask --resume，不重发问题。新文件前、测试后和完成前执行: orca orchestration check --terminal %s --json；处理整批后只对同一 worker handle 执行 check --ack <delivery_id> 并排空后续批次；consumer_fenced 或 dispatch_inactive 时立即停止且不发 worker_done。其余仍按已注入任务执行。' \
        "$task_id" "$dispatch_id" "$completion_command" "$terminal_handle" "$terminal_handle"
