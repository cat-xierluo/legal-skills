...
- Shell is also fail-closed: use only the narrow built-in lifecycle/read commands or the exact `Allowed Shell Commands` emitted by spawn. The built-in `sed` exception is deliberately narrow: only `sed -n '<numeric-range>p' <single-file>` is read-only; substitutions, `w`/`e`, multi-file and other forms still require an exact allowlist entry. `--verify-cmd` grants execution authority only after install-like commands such as `npx`/`npm exec`/`pnpm dlx` are rejected.
