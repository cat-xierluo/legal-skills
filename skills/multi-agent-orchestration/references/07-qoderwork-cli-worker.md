...
## 10. qoder spawn 实战坑（2026-07-05 PM 诊断修复）

> 触发：Wave3 qoder worker spawn 后 tmux 立即退（0 commit）。PM 深挖两层根因 + 修复。

### 10.1 路径空格 + tmux 引号吞 → env 127

qoder BIN = `/Applications/QoderWork CN.app/Contents/Resources/bin/qoderclicn`（**含空格**）。render-runtime-profile 的 `quote_words` 用 `printf %q` 生成反斜杠转义（`/Applications/QoderWork\ CN.app/...`），但 **tmux `new-session` 的 command parser 吃掉反斜杠** → 路径拆词 → `env: /Applications/QoderWork: No such file or directory` exit 127。

codebuddy 无此问题（`WorkBuddy.app` 无空格）。

**修复**（spawn-worker.sh，自动检测）：COMMAND 含空格 → 写 `launch.sh`（内 `exec bash -c %q`，bash 下正确解析 %q 转义），tmux 只跑 `bash launch.sh`（无空格），绕过 tmux command 解析。通用：任何含空格路径/特殊字符的 COMMAND 都自动受益（不靠本机 symlink）。

### 10.2 detached 交互模式 exit 42

launch.sh 修路径后，qoder 启动但 **exit 42**（静默，无 stdout/stderr/debug 输出）。根因：qoder 纯 detached tmux 交互模式（无初始 prompt）检测"无初始输入"立即退 42。

**修复**（render-runtime-profile.sh qoder case）：交互模式加 `-i "ready"`（`--prompt-interactive` 占位 prompt），让 qoder 启动 REPL；PM 后续 tmux `send-keys` 投递真任务。实测：无 `-i` → exit 42；`-i ready` → qoder 启动 REPL（`Ready (scope-guard-test)`）。

codebuddy 无此问题（交互模式 detached 不退）。

### 10.3 qoder 认证（本机，非 skill 问题）

`-i` 启动后 qoder 可能显示 `Authenticating / Waiting for authentication`——这是 qoder 桌面端登录态/CLI 认证（本机问题，确保 QoderWork CN 桌面端已登录）。batch `-p` 模式复用登录态（实测工作），交互模式 `-i` 走完整认证流。

### 10.4 qoder spawn 完整修复链

| 层 | 问题 | 修复 |
|---|---|---|
| 路径 | BIN 含空格 + tmux 引号吞 → env 127 | spawn-worker 自动 launch.sh（§10.1） |
| 启动 | detached 交互无初始 prompt → exit 42 | render qoder 加 -i ready（§10.2） |
| scope | 越界（-y 下） | scope-guard PreToolUse hook + wrapper（§9） |
| runtime | trust/dialog/prompt | trust_auto + permission_auto + -y 默认（既有 fix） |

四层都修后，qoder spawn 应零手动干预 + 不崩 + 不越界。

---

> **版本记录**：
> - 2026-07-05：新增 §10 qoder spawn 实战坑（路径空格→launch.sh + detached exit 42→-i，PM 诊断修复）；§9 权限与 scope 控制（PreToolUse hook unbypassable，scope-guard 决定性方案）。
