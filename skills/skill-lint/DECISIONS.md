...
## D-2026-07-25-01 具体失效预筛采用“确定性静态规则 + 动态证据门禁”分层

- 背景：现有 ISG 能阻止证据不足的 Skill 被判稳定，却不能自动解释 checker 崩溃被吞、状态副作用未落地、配置未进入执行路径、Git 完成回执失真等具体实现根因；大量 Skill 全量审查需要先低成本定位。
- 决策：新增候选代码零执行的 HFA/HRA 静态审查层，finding 绑定具体文件、行号、证据、影响和修正建议；单 Skill 与集合使用同一规则，批量零发现 fail-closed。ISG `assess` 组合结构性约束缺口与具体实现 finding，但不改变正式 verify/签名回执协议。
- 误报边界：生产与测试使用不同规则；同一规则按文件聚合；未知参数只在 argv 解析区识别；结果消费管道不冒充 checker 退出丢失；视觉语义只读取规范源并要求硬约束局部共现。
- 动态边界：自动 fault probe 不在本版本执行。后续实现必须要求用户确认可信候选、复制到临时目录、使用最小环境，且禁止自动安装和联网。
- 校准：把文档治理 Skill 的已确认失效抽象为去具体化 bad/good twins，不复制真实项目内容；真实 Skill 仅作只读抽测。
