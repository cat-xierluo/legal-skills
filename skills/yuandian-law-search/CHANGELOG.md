...
### 改进（references/07 文档，落地独立 review 的 nit）

- **§3.2 新增「已有法律分析报告」三栏规则**：`prior_report_sources` 明确拆为 `report_facts` / `report_conclusions` / `hypotheses_to_verify`；区分「报告援引的法源」（可作检索线索，不降级）与「报告作者的法律判断」（必须降级为假设）。补齐 schema 缺定义、但评测 case-03 已按其判分的核心验收点。
- **§3.1 高管 row 补归入权**：违反忠实义务时可主张归入权（所得收入收归公司）。
- **§8 近邻清单补「信息网络传播权」**：商业诋毁/不正当竞争 ≈ 侵害作品信息网络传播权（达人发视频涉著作权路径）。
- **§8 `must_exclude` 写法指导**：每项一个独立近邻，避免斜杠合并（如「制作/搬运」拆两项），防 judge 漏读。
- **§8 加 references/02 交叉引用**：指向场景 4-5 红线。
- **§6 明确「1 轮」定义**：1 轮 = 1 次交互回合，单回合最多 3 个会改变检索路径的核心问题；不得套用 5 字段争点表全字段追问（修 Qwen3.5 把 baseline 6 问误读为超轮的诱因）。
