...
```bash
# 法条关键词扩展检索
python3 scripts/yd_search.py keyword "刑事案件 管辖权" --expand "知识产权管辖,级别管辖,专门管辖" --search-mode or

# 案例关键词扩展检索
python3 scripts/yd_search.py case "买卖合同 瑕疵担保" --expand "质量纠纷,违约责任" --search-mode or

# 法规关键词扩展检索
python3 scripts/yd_search.py regulation "民法典 合同" --expand "买卖合同,租赁合同" --search-mode or
```
...
**第一阶段（广撒网）**：
```bash
python3 scripts/yd_search.py keyword "刑事案件 管辖权 级别管辖 地域管辖 专门管辖" --search-mode or --sxx 现行有效
```
...
**第二阶段（精提炼）**：
```bash
python3 scripts/yd_search.py keyword "级别管辖 中级法院" --search-mode and --sxx 现行有效
python3 scripts/yd_search.py keyword "地域管辖 犯罪地" --search-mode and --sxx 现行有效
```
