...
- **普通 Markdown 表题自动居中**：`**表 X-Y：...**` 与不带粗体标记的同类表题自动水平居中、取消首行和左缩进；只覆盖段落对齐与缩进，不改变原有字号或粗体。显式 `<div align="center">` 路径保持兼容，图注既有小一号样式不变。

### 验证
- 测试先行复现六列长表头合计 `17.84 cm > 15.00 cm`、表格 `tblW=auto/0` 与 grid/tcW 不同源、普通表题两端对齐且首行缩进三类失败；新增算法、端到端 OOXML 和表题回归后，全量 15 项测试通过。
...
- 真实第 10 章临时转换中，表 10-5 表题由 `jc=both / firstLine=480` 变为 `jc=center / firstLine=0 / left=0`，粗体与 `24 half-points（12 pt）` 字号保持不变。临时 DOCX 均保存在 `/tmp`，未提交仓库。
