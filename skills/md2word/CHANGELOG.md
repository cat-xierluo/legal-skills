...
### 新增
- **自然语言示例框（DEC-015）**：新增 ` ```example ` 围栏，用于 AI 原始输出与自然语言范例。转换器以单单元格表格写入浅灰底纹及真实上下/左右内边距，文本沿用书籍正文的字体、12pt 与 1.5 倍行距；框内取消首行缩进和段前段后，避免紧贴灰色边缘或出现代码块式的紧凑节奏。

### 兼容性
- ` ```text ` 及其他代码围栏继续走既有等宽、9pt、1.2 倍行距的 `code_block` 路径，不因示例框新增而改变。

### 验证
- 新增端到端回归，断言 `example` 产生单表格、`#F2F2F2` 底纹、100/100/140/140 twips 内边距、正文 12pt 与 1.5 倍行距，并同时断言 `text` 仍为 Courier New、9pt、1.2 倍行距；全量回归 17/17 通过。
