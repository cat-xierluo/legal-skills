...
- **验收证据**：RED 两项分别复现正文吞下划线与表格预判误报；GREEN 全量 19/19 通过。真实 ch14 四字段、ch12 `main_chart_type`、ch04 `API_SERVER_KEY` 均在 `/tmp` DOCX 中完整存在且目标 run 无 `w:i`；QuickLook 最小首屏 fixture 经目检通过。`quick_validate.py` 因 origin/main 已有且项目规范要求的 `author`、`homepage`、`version` frontmatter 键退出 1，记为 `NOT_VERIFIED`。
