...
普通 Markdown 表题 `**表 10-5：...**` 没有进入既有图注分支，因而按正文输出为两端对齐和 24 pt 首行缩进。显式 `<div align="center">` 虽可规避，但 canonical Markdown 不应依赖人工包裹才能得到基本表题对齐。
...
4. 普通表题识别沿用图注的编号形式，但只覆盖段落水平居中、首行缩进和左缩进；不复用图注 10 pt 字号规则，不改变 Markdown 粗体解析。显式 HTML 居中路径和图注行为均保持不变。
...
### 验证
- RED：六列 fixture 的 `_calc_column_widths` 返回 17.84 cm；端到端 grid 合计 9752 twips，超过向下量化后的 8503 twips 正文区；普通表题为 `JUSTIFY`。
- GREEN：新增三项回归后，全量 15 项测试通过；六列计算宽度铺满且不超过正文区，端到端 `tblW/grid/tcW` 完全一致，普通表题居中且缩进为 0，并保留粗体与正文 12 pt 字号。
- 真实 ch07 表 7-15：`10115 twips（17.8417 cm）→8503 twips（14.9982 cm）`；真实 ch10 表 10-5 表题：`jc=both / firstLine=480 → jc=center / firstLine=0 / left=0`，run 仍为粗体、24 half-points。
