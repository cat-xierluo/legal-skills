...
## [DEC-015] - 2026-08-25 - 用语义围栏区分自然语言输出示例与代码块

### 背景
书籍第十一章的 AI 原始输出以 ` ```text ` 围栏写入。该语义被转换器当作代码，因而使用 9pt 等宽字体、1.2 倍行距、无真实上下内边距；当内容实际是自然语言时，阅读密度明显高于相邻正文。

### 决策
1. 新增 ` ```example ` 语义围栏，专用于自然语言输出、AI 原始输出和待评议范例；不将既有 `text` 围栏作全局替换。
2. `example` 输出为一张正文宽度的单单元格无边框表格。单元格写入 `#F2F2F2` 浅灰底纹，以及 5pt 上下、7pt 左右的真实内边距；不用空行或段落边框伪造留白。
3. 示例内容复用正文的字体、12pt 和 1.5 倍行距，并清除框内首行缩进、段前和段后；因此它与正文保持相近阅读节奏，仅以灰底标识“这是一个示例”。
4. `text` 和其他代码围栏继续使用既有 `code_block` 配置（Courier New / 9pt / 1.2 倍行距），保持真实代码的紧凑、等宽呈现。

### 方案取舍
- 不直接放宽 `text` 的行距或字号：全书已有 `text` 围栏中可能包含真正的文本型代码、模板或终端内容，批量改变会扩大视觉影响面。
- 不复用 Markdown `>` 引用块：引用块承担引文语义且当前没有底纹；AI 原始输出需要明确的示例语义和稳定的灰色背景。
- 不使用逐段底纹：单元格能稳定提供四向内边距，避免多行内容的段落底纹在 Word 中贴边或断裂。

### 验证
- 端到端回归用同一 fixture 同时生成 `example` 与 `text`：前者产生一张表格、浅灰底和显式 `tcMar`，文本为正文 12pt / 1.5；后者仍为 Courier New 9pt / 1.2，不进入表格。
- `python3 -m unittest skills/md2word/scripts/test_regressions.py -v`：17/17 通过。

### 影响与回退
- 只有显式标为 `example` 的围栏走新增路径；所有现有书稿与预设在未改标记时输出不变。
- 如需回退，删除 `example` 分派与相关配置即可；不应通过改写 `code_block` 默认值回退。
