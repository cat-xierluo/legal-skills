...
- 设计规范新增**「元素删除后的画布回收」纪律**(设计演进负债预防):删除图内可见元素(历史标记标题/说明条/废弃节点)必须同步收紧 viewBox 与显式 height,使留白回归口径(首内容距上缘 24-40px、四周 40px)。来源:书籍项目作者 2026-08-29 对 T248(全书留白压缩,图 7-2/8-4/11-7/13-1)的核实归因——早期顶部「标记标题」设计被移除后未回收画布,多轮修改累积为技术负债后需全书统一排查;规范把约束点放在**删除动作的当下**,scan 只能拦「边距不足」,「留白过大」与有意分组留白无法机械区分。
