...
多轮书籍 SVG 复核显示，语法正确、箭头落点正确仍会出现四类读者可见问题：为“简化”误删带反馈或状态含义的虚线；同一分叉因微小缝隙看似断连；标签栏叠加圆角而像两张卡；编号或制图提示压入正文，图例也未真正解释给读者。层级图还常以长箭头替代紧凑布局，空白被误读为层级。

### 决策

1. 有流程、反馈、复核、状态或条件含义的实线 / 虚线 / 点线属于图的内容，不得为删繁而删除；冲突优先靠重路由、节点调整和读者标签解决。
2. 同一关系的主干与支线必须视觉连续；相邻层级箭头的可见线身优先 16–32px，超过 48px 时优先压缩布局。真实中间步骤 / 注释是唯一的合理长连接理由。
3. 深色标题 + 浅色正文是同一卡片时，以单外轮廓和仅上圆角的标题栏表达；步骤编号有独立上方带，空间不足则增加卡高。
4. 图例只承载读者理解关系所需的解释，不承载作者、AI 或制图流程沟通。
5. 将上述内容加入 style guide、review checklist、SKILL 工作流和成功标准；它们需要受控渲染后的视觉目检，不能只由 XML 或 producer contract 判定。

### 兼容性

这是新生成和新修订 SVG 的通用设计 / 审查规则，不批量重绘历史资产。项目有更严格的 canonical 时，项目规则优先。
