...
### 5.0 配色总则（v1.7.0 起条件化拆分，按模板语义分类）
...
>
> **项目级规范优先（v1.8.6 起，防漂移）**：生成配图前**先读项目的 `figures/FIGURES-OUTLINE.md`**（或等价配图大纲/风格规范）。若该书在其中锁定了 canonical 配色（例如《法律 AI Skill 实战》锁定 `#2C5282` 主系 + P1 雾蓝柔和变体，灰阶做模块填充、`#2C5282` 蓝只做点缀强调、禁止大面积蓝填充），**从其规定**——本 skill 的 P1-P8 / G1-G4 是通用兜底，不得与项目 canonical 冲突。这是防止"不同批次补图风格漂移"的硬约束（该书曾因未对齐项目规范，经历雾蓝→Tailwind→回退原始三轮返工，见该项目 FIGURES-OUTLINE.md「配图风格规范」）。
