...
`bash tests/run_e2e.sh` — 内容断言与 68 编号冒烟 + 长文本矩阵快速合同 + 113 棵树 DOCX 版式静态门禁 + 21 类文书真实 PDF 渲染 + 版式违规正反例。

68 棵主文书的发布级长文本矩阵独立运行，不塞进日常 E2E：

```bash
python tests/smoke_render_longtext.py \
  --rounds 3 --require-full \
  --json-output tests/output/longtext-report.json \
  --evidence-dir tests/output/longtext-evidence
```

该入口为每棵主文书生成确定性的长当事人、长诉请和长事实输入，逐类先预热一次，再连续计量三轮；每轮复用正式 DOCX/PDF 门禁，并记录页数、连续页码、空白页、字形存活、候选内容哈希和稳定指纹。证据目录必须为空，拒绝复用旧产物；定位波动时可加 `--evidence-all-rounds` 保留每轮 DOCX、PDF 与关键页截图。
...
- 68 棵主文书长文本矩阵已在正式版 LibreOffice 26.2.3.2 下完成逐类预热和三轮计量：204/204 次渲染通过，0 失败、0 不稳定，输入哈希、候选 DOCX 内容哈希与结构指纹均一致，当前环境状态为 `DOMAIN_VERIFIED`。
- 45 棵答辩状/第三人意见陈述书尚未进入同等级长文本矩阵；不得把 68 棵主文书结果扩大解释为 113 棵模板全量极端长度已验证。
...
- 当前版本：`0.16.0`（2026-09-22）
- 0.16.0：新增 68 主文书长当事人/诉请/事实真实渲染矩阵与结构稳定指纹；修复长字段继承巨大右缩进导致裁切、终端空分节尾页、合法跨页网格细化、相邻单元格误报及 LibreOffice 字体替代漂移；300 字以上长行允许自然跨页；完整 204 次计量 0 失败、0 不稳定，状态为 `DOMAIN_VERIFIED`
