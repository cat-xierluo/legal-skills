...
以下特征出现时，应怀疑为AI化表述：
- 三个以上并列项，且每一项都以相同虚词开头（`去/换/是/也是/就更`）
...
**问题分析**：
- AI 用程度副词（很、非常）为模糊表达打补丁，显得程式化
- 程度副词+抽象动词的结构尤其显得堆叠（"很依赖人去..."）
- 真人表达倾向于直接说具体感受，或用更精确的动词替代程度副词

**改写方向**：
- `有个很直接的感觉` → `有个直接的体会`
- `整个过程很吃人` → `整个过程把人绑得死死的`
- `很依赖人去记、去追` → `依赖人去记、去追，带宽全被占用`
- 用口语化动词替代程度副词，或直接陈述具体状态
...
**问题分析**：
- AI 用大词描述感受，显得用力过度
- 真人描述感受时倾向于直接说"有意思"、"不一样"、"值得注意"
- 用大词描述感受反而显得不真实，像在刻意强调

**改写方向**：
- `Git 真正让我震动的` → `Git 真正让我觉得有意思的`
- `这让我很震撼` → `这让我注意到的` 或直接描述注意到了什么
