...
6. **高价值素材识别**：
   ...
   - 每批先读取 `status.maximum_new_material_count_for_next_batch`，新增素材不得超过该值；整份账本仍不得超过 `status.maximum_material_count`，即 `max(60, ceil(content block 数 × 0.5))`。正常长课通常约 40—80 项。额度不足时先合并同一观点、连续操作阶段或同一 skip 理由，不得把大量微素材留给正文阶段承担
