...
- **ONNX 加速模式**：支持 `paraformer-onnx` 与 `SenseVoice-Small ONNX`
- **单人快速模式**：`--fast` / `"fast": true` 自动切到 SenseVoice，并关闭 diarization
...
首次需要运行 ONNX 快速模式时，直接执行：

```bash
python3 scripts/setup.py
```

即可同时安装 `funasr-onnx` 及其依赖；`SenseVoiceSmall` 模型会在首次使用时自动下载。
...
> 单人讲课/语音可传 `"fast": true` 或 `"model": "sensevoice"`，服务会自动走快速路径。
