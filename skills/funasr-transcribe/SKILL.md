...
```bash
cd <skill目录> && python scripts/server.py --idle-timeout 600 &
```
...
# 提取视频关键帧截图（PPT幻灯片）
python scripts/transcribe.py /path/to/video.mp4 --slides

# 自定义场景检测阈值（值越低越灵敏，默认27.0）
python scripts/transcribe.py /path/to/video.mp4 --slides --slide-threshold 20.0
```
...
python scripts/transcribe.py /path/to/audio.m4a --auto-summary

# 完整流程：说话人分离 + 自动总结
python scripts/transcribe.py /path/to/meeting.m4a --diarize --auto-summary
```
...
# 禁用自动总结（只输出提示词）
python scripts/transcribe.py /path/to/audio.m4a --no-summary
```
...
# 禁用说话人分离
python scripts/auto_transcribe.py /path/to/audio.aac --no-diarize
